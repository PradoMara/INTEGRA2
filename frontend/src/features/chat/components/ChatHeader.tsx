...
export function ChatHeader({ chatActivo }: { chatActivo: Chat | null }) {
  return (
    <motion.header 
      className="h-[64px] flex items-center px-6 shadow-md"
    ...
    >
      <div className="flex items-center gap-3">
        <motion.div 
          className="h-10 w-10 rounded-full bg-white grid place-items-center shadow"
          whileHover={{ scale: 1.05 }}
          transition={{ type: 'spring', stiffness: 300 }}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="text-[#0B3A66]">
